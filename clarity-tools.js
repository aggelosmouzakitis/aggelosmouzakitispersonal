...
function cTopWeighted(arr) {
  return arr.length ? 0.6 * cMax(arr) + 0.4 * cMean(arr) : 0;
}
function cClamp(n) {
...
function computeDimension(data, dimKey, answers) {
  ...
  data.questions.forEach(function (q) {
    ...
    var opt = q.options[a];
    if (!opt) return;
    var maxS = cMax(q.options.map(function (o) {
      return o.s;
    }));
    if (maxS <= 0) return;
  ...
  var wsum = vals.reduce(function (a, b) {
    return a + b.w;
  }, 0);
  var acc = vals.reduce(function (a, b) {
    return a + b.v * b.w;
  }, 0);
  return {
    score: cRound(cClamp(acc / wsum * 100)),
    valid: vals.length,
    count: count
  };
}
...
function computeDims(data, answers) {
  ...
  data.dimensions.forEach(function (d) {
    ...
    dims[d.key] = {
      ...
      group: d.group || null,
      show: d.show !== false,
      minValid: d.minValid || 1,
      score: r.score,
      valid: r.valid,
      count: r.count,
      bracket: idx,
      labelShort: labels[idx],
...
function clarityHelpers(dims) {
  return {
    ...
    score: function (key) {
      return dims[key] ? dims[key].score : null;
    },
    ...
    sortedBy: function (filter) {
      return Object.keys(dims).map(function (k) {
        ...
        return d.answered && (!filter || filter(d));
      }).sort(function (a, b) {
        return b.score - a.score;
      });
    },
    labelShort: function (type, idx) {
      return (CLARITY_LABELS[type] || CLARITY_LABELS.problem)[idx];
    }
  };
}
...
function clarityScore(data, answers) {
  ...
  function headline(cfg) {
    if (!cfg) return null;
    var raw = cfg.compute ? cfg.compute({
      dims: dims,
      answers: answers,
      H: H
    }) : 0;
    ...
    return {
      ...
      name: cfg.name,
      meaning: cfg.meaning,
      type: cfg.type || 'problem',
      unit: cfg.unit || '/100',
      score: score,
      bracket: idx,
      bracketLabel: br.label,
      bracketDesc: br.desc,
      followNote: cfg.followNote || null
    };
  }
...
function clarityBuildReport(data, answers, result, person) {
  ...
  var head = function (h) {
    if (!h) return;
    line(h.name + ': ' + h.score + h.unit + ' — ' + h.bracketLabel);
  };
  line('── HEADLINE SCORES ──');
  head(result.overall);
  head(result.overall2);
  line('');
  if (result.interp.secondary) {
    line('── SECONDARY RESULT ──');
    line(result.interp.secondary);
    line('');
  }
  if (result.interp.modifiers && result.interp.modifiers.length) {
    line('── MODIFIERS ──');
    result.interp.modifiers.forEach(function (m) {
      line('• ' + m);
    });
    line('');
  }
  ...
  data.questions.forEach(function (q, i) {
    var a = answers[q.id];
    var ansTxt;
    if (a === 'na') ansTxt = 'N/A';else if (a == null) ansTxt = '—';else ansTxt = q.options[a] ? q.options[a].t : '—';
    line(i + 1 + '. ' + q.text);
...
function claritySubmit(data, answers, result, person, done) {
  ...
  if (window.emailjs) {
    try {
      emailjs.send(CLARITY_EMAILJS_SERVICE, CLARITY_EMAILJS_TEMPLATE, payload).then(finish).catch(function (e) {
        console.error('Clarity email error:', e);
        finish();
      });
...
function clarityStyles(mob) {
  return {
    page: {
      maxWidth: 820,
      margin: '0 auto',
      padding: mob ? '1.75rem 1.25rem 5rem' : '3.5rem 2.5rem 6rem',
      color: '#282726',
      fontFamily: 'inherit'
    },
    eyebrow: {
    ...
    naChoice: function (sel) {
      return {
        ...
        borderRadius: '10px',
        background: sel ? 'rgba(4,120,87,.08)' : '#fafafa',
        color: '#555',
        fontFamily: 'inherit',
        fontSize: '15px',
        lineHeight: 1.5,
...
function ClarityProgress({
  ...
  return React.createElement('div', {
    ...
    style: C.eyebrow
  }, 'Question ' + (i + 1) + ' of ' + total), React.createElement('p', {
    style: C.eyebrow
  }, pct + '% complete')), React.createElement('div', {
    style: C.progLine
  }, React.createElement('div', {
    style: C.progFill(pct)
  })));
}
...
function ClarityBar({
  ...
  if (dim.score == null) {
    ...
    }, 'Not enough answers')), React.createElement('p', {
      style: {
        fontSize: '14px',
        color: '#777',
        lineHeight: 1.55,
    ...
    }, 'You marked this area as not applicable, so it is excluded from the result.'));
  }
  ...
  })), dim.copy ? React.createElement('p', {
    style: {
      fontSize: '14.5px',
      color: '#555',
      lineHeight: 1.55,
  ...
  }, dim.copy) : null, dim.lowConfidence ? React.createElement('p', {
    style: {
      fontSize: '13px',
      color: '#999',
      lineHeight: 1.5,
  ...
  }, 'Based on few answers, so read this as a weaker signal.') : null);
}

// A headline score block (overall, or a paired headline like fit / readiness).
function ClarityHeadline({
  head,
  C,
  compact
}) {
  ...
  }, head.name), React.createElement('div', {
    style: {
      display: 'flex',
      alignItems: 'baseline',
      gap: '.5rem',
  ...
  }, head.score), React.createElement('span', {
    style: {
      fontSize: '16px',
      color: '#999',
      fontWeight: 600
    }
  }, head.unit)), React.createElement('div', {
    style: {
      fontSize: '15px',
      fontWeight: 700,
      color: tone,
  ...
  }, head.bracketLabel), head.meaning ? React.createElement('p', {
    style: {
      fontSize: '13px',
      color: '#888',
      lineHeight: 1.5,
  ...
  }, head.meaning) : null, !compact && head.bracketDesc ? React.createElement('p', {
    style: {
      fontSize: '15px',
      color: '#444',
      lineHeight: 1.6,
  ...
  }, head.bracketDesc) : null, head.followNote ? React.createElement('p', {
    style: {
      fontSize: '13.5px',
      color: '#9B2C2C',
      lineHeight: 1.5,
...
function ClarityResult({
  ...
  var shown = data.dimensions.filter(function (d) {
    return d.show !== false;
  }).map(function (d) {
    return result.dims[d.key];
  }).filter(Boolean);
  ...
  React.createElement('h1', {
    style: C.h1
  }, 'Your result'), React.createElement('p', {
    style: {
      ...C.p,
  ...
  }, interp.primary), interp.secondary ? React.createElement('p', {
    style: {
      ...C.p,
      color: '#333'
    }
  }, interp.secondary) : null,
  // 2. Overall score(s)
  twoHead ? React.createElement('div', {
    style: {
      display: 'flex',
      gap: '1rem',
  ...
  }, React.createElement(ClarityHeadline, {
    head: result.overall,
    C: C,
    compact: true
  }), React.createElement(ClarityHeadline, {
    ...
    head: result.overall,
    C: C
  })), twoHead && result.overall.bracketDesc ? React.createElement('p', {
    style: {
      fontSize: '15px',
      color: '#555',
      lineHeight: 1.6,
  ...
  }, result.overall.bracketDesc) : null, twoHead && result.overall2 && result.overall2.bracketDesc ? React.createElement('p', {
    style: {
      fontSize: '15px',
      color: '#555',
      lineHeight: 1.6,
  ...
  }, result.overall2.bracketDesc) : null,
  // 3. What is driving the score
  interp.driving && interp.driving.length ? React.createElement(ClaritySection, {
    title: 'What is driving the score',
    C: C
  }, clarityList(interp.driving, C)) : null,
  // 4. Your breakdown — grouped when the assessment defines groups, else flat.
  React.createElement(ClaritySection, {
    title: 'Your breakdown',
    C: C
  }, data.breakdownGroups && data.breakdownGroups.length ? data.breakdownGroups.map(function (g, gi) {
    return React.createElement('div', {
      key: gi,
      style: {
        marginBottom: '1.6rem'
    ...
    }, g.title), g.note ? React.createElement('p', {
      style: {
        fontSize: '13px',
        color: '#888',
        lineHeight: 1.5,
    ...
    }, g.note) : null, g.keys.map(function (k) {
      return result.dims[k] ? React.createElement(ClarityBar, {
        key: k,
        dim: result.dims[k],
        C: C
      }) : null;
  ...
  }, data.breakdownNote || (result.overall && result.overall.type === 'fit' ? 'Higher scores mean more supporting evidence.' : 'Higher scores mean stronger evidence of a problem in that area.')), shown.map(function (dm) {
    return React.createElement(ClarityBar, {
      key: dm.key,
      dim: dm,
      C: C
    });
  ...
  interp.healthy && interp.healthy.length ? React.createElement(ClaritySection, {
    title: 'What looks healthy',
    C: C
  }, clarityList(interp.healthy, C)) : null,
  // 6. What else matters
  interp.elseMatters ? React.createElement(ClaritySection, {
    title: 'What else matters',
    C: C
  }, Array.isArray(interp.elseMatters) ? clarityList(interp.elseMatters, C) : React.createElement('p', {
    style: {
      fontSize: '16px',
      color: '#333',
      lineHeight: 1.65,
  ...
  }, interp.elseMatters)) : null,
  // 7. The part worth challenging
  interp.challenge ? React.createElement(ClaritySection, {
    title: 'The part worth challenging',
    C: C
  }, React.createElement('p', {
    style: {
      fontSize: '16px',
      ...
      margin: 0,
      paddingLeft: '.9rem',
      borderLeft: '2px solid ' + CL_ACC
    }
  }, interp.challenge)) : null,
  // 8. What to test next
  interp.testNext && interp.testNext.length ? React.createElement(ClaritySection, {
    title: 'What to test next',
    C: C
  }, clarityList(interp.testNext, C)) : null,
  // 9. Important context
  interp.context ? React.createElement(ClaritySection, {
...
function ClarityTool({
  ...
  var React_ = React;
  var useState = React_.useState,
    useEffect = React_.useEffect,
    useRef = React_.useRef;
  ...
  var choose = function (q, i, optIndex) {
    ...
    clarityTrack('assessment_question_progress', {
      assessment: slug,
      question: i + 1,
      total: total
    });
  ...
  var submit = function () {
    ...
    if (!valid) {
      setErr('Please enter a valid email so we can send your result.');
      return;
    }
    setErr('');
    setSending(true);
  ...
  if (screen === 'intro') {
    ...
    }, data.intro.eyebrow), React.createElement('h1', {
      style: C.h1
    }, data.title), data.intro.paras.map(function (p, i) {
      return React.createElement('p', {
        key: i,
        style: C.p
      }, p);
    }), data.intro.note ? React.createElement('p', {
      style: {
        fontSize: '15px',
        color: '#555',
        lineHeight: 1.6,
  ...
  if (typeof screen === 'number') {
    ...
    }), q.na ? React.createElement('button', {
      className: 'opt-btn',
      style: C.naChoice(sel === 'na'),
      onClick: function () {
        choose(q, i, 'na');
      }
    }, q.naText || 'Not applicable to me') : null, React.createElement('div', {
      style: {
        marginTop: '1.8rem'
      }
  ...
  if (screen === 'email') {
    return React.createElement('div', {
      ...
      disabled: sending,
      onClick: submit
    }, sending ? 'Working…' : 'See my result →')), React.createElement('p', {
      style: {
        fontSize: '13px',
        color: '#999',
        lineHeight: 1.6,
