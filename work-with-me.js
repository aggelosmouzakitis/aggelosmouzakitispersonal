...
var FM_TEXT = {
  ...
  habit: {
    label: 'Habit',
    sub: '/ Protection',
    q: FORCES_AGAINST[0].q,
    c: FM_CORAL
  },
  anxiety: {
    label: 'Anxiety',
    q: FORCES_AGAINST[1].q,
    c: FM_CORAL
  }
};
...
function WmForcesMap() {
  ...
  R.useEffect(function () {
    ...
    function g() {
      return mobileRef.current ? FM_G.m : FM_G.d;
    }
    var f = function (n) {
      return n.toFixed(2);
    };
    ...
    var amt = function (v, k) {
      return M.clamp01((v[k] - 0.5) * 2);
    };
    ...
    function paint() {
      var G = g(),
        v = S.v || fmTarget('default', G),
        ms = S.e * FM_ENTRANCE;
      var n = nodes.current;
      var cx = G.cx0 + v.sx;
      var set = function (node, k, val) {
        if (node) node.setAttribute(k, val);
      };
      FM_KEYS.forEach(function (key) {
        ...
        [-2, -1, 0, 1, 2].forEach(function (k) {
          var core = Math.abs(k) <= 1,
            j = k + 1;
          var dp = core ? seg(ms, FM_DRAW[key] + j * 60, 360) : 1;
          var up = key === 'push' || key === 'pull';
          var yF = (up ? G.yFU : G.yFD) + k * G.sF,
            yN = (up ? G.yNU : G.yND) + k * G.sN;
          var nearL = cx - G.R - G.gap,
            nearR = cx + G.R + G.gap;
          var farL = Math.max(0, G.farL - G.ext * a),
            farR = Math.min(G.axR, G.farR + G.ext * a);
          var x1, y1, x2, y2;
          if (key === 'push') {
            x1 = farL;
            y1 = yF;
          ...
          if (ln) {
            ...
            set(ln, 'y2', f(y2));
            set(ln, 'stroke', c);
            set(ln, 'stroke-width', f(sw));
            set(ln, 'opacity', op.toFixed(3));
            set(ln, 'stroke-dasharray', f(len + 2));
            set(ln, 'stroke-dashoffset', f((len + 2) * (1 - dp)));
          }
          if (k === 0) {
            var ux = (x2 - x1) / len,
              uy = (y2 - y1) / len,
            ...
            var hl = G.head[0],
              hw = G.head[1];
            var pts = [[x2 - hl * ux + hw * nx, y2 - hl * uy + hw * ny], [x2, y2], [x2 - hl * ux - hw * nx, y2 - hl * uy - hw * ny]].map(function (p) {
              return f(p[0]) + ',' + f(p[1]);
            }).join(' ');
            var hd = n['h_' + key];
            if (hd) {
      ...
      var ldLen = Math.abs(l2 - l1) + 2,
        rdLen = Math.abs(r2 - r1) + 2;
      ...
      set(n.ax_stop, 'y1', G.y0 - tick);
      set(n.ax_stop, 'y2', G.y0 + tick);
      set(n.ax_block, 'stroke-dasharray', G.dash);
      set(n.you_ghost, 'cx', G.cx0);
      set(n.you_ghost, 'cy', G.y0);
      set(n.you_ghost, 'r', G.R);
      set(n.you_c, 'cy', G.y0);
      set(n.you_c, 'r', G.R);
      ...
      set(n.ax_l, 'stroke-dasharray', f(ldLen));
      set(n.ax_l, 'stroke-dashoffset', f(ldLen * (1 - eL)));
      set(n.ax_lhab, 'opacity', amt(v, 'habit').toFixed(3));
      set(n.ax_r, 'x1', f(r1));
      set(n.ax_r, 'x2', f(r2));
      set(n.ax_r, 'stroke-width', f(2 + 1.5 * amt(v, 'pull')));
      set(n.ax_r, 'stroke-dasharray', f(rdLen));
      set(n.ax_r, 'stroke-dashoffset', f(rdLen * (1 - eR)));
      [['ax_mask', r1, b2], ['ax_block', r1, b2]].forEach(function (p) {
        set(n[p[0]], 'x1', f(p[1]));
        set(n[p[0]], 'x2', f(p[2]));
        set(n[p[0]], 'opacity', amt(v, 'anxiety').toFixed(3));
      });
      set(n.ax_stop, 'x1', f(b2));
      set(n.ax_stop, 'x2', f(b2));
      ...
      var arc = function (a1, a2) {
        var r1a = a1 * Math.PI / 180,
          r2a = a2 * Math.PI / 180;
        return 'M' + f(cx + arcR * Math.cos(r1a)) + ' ' + f(G.y0 + arcR * Math.sin(r1a)) + ' A' + arcR + ' ' + arcR + ' 0 0 1 ' + f(cx + arcR * Math.cos(r2a)) + ' ' + f(G.y0 + arcR * Math.sin(r2a));
      };
      var left = arc(150, 210),
        right = arc(-30, 30);
      set(n.arc_push, 'd', left);
      set(n.arc_push, 'opacity', amt(v, 'push').toFixed(3));
      set(n.arc_habit, 'd', left);
      set(n.arc_habit, 'opacity', amt(v, 'habit').toFixed(3));
      set(n.arc_pull, 'd', right);
      set(n.arc_pull, 'opacity', amt(v, 'pull').toFixed(3));
      set(n.arc_anx, 'd', right);
      set(n.arc_anx, 'opacity', amt(v, 'anxiety').toFixed(3));
      ...
      var youIn = seg(ms, 1250, 350),
        sc = 0.6 + 0.4 * youIn;
      set(n.you_trail, 'x1', f(Math.min(G.cx0, cx)));
      set(n.you_trail, 'x2', f(Math.max(G.cx0, cx)));
      ...
      set(n.you_g, 'transform', 'translate(' + f(cx) + ' ' + G.y0 + ') scale(' + sc.toFixed(3) + ') translate(' + f(-cx) + ' ' + -G.y0 + ')');
      set(n.you_c, 'cx', f(cx));
      set(n.you_t, 'x', f(cx));
      set(n.you_t, 'opacity', seg(ms, 1400, 200).toFixed(3));

      // Labels, questions and the intervention chips.
      FM_KEYS.forEach(function (key) {
        var i = v[key],
          into = seg(ms, FM_LABEL_IN[key], 400);
    ...
    function goTo(next, instant) {
      ...
      S.active = next;
      setActive(next);
      if (instant || !S.v || M.reduced) {
        S.v = to;
        paint();
        return;
      }
      var from = Object.assign({}, S.v),
        t0 = performance.now();
      var step = function (now) {
        var k = M.clamp01((now - t0) / 520),
          ek = M.travel(k),
          v = {};
        Object.keys(to).forEach(function (key) {
          v[key] = from[key] + (to[key] - from[key]) * ek;
        });
        S.v = v;
        paint();
    ...
    el.__fmSet = function (key, sticky) {
      S.sticky = !!sticky;
      goTo(key);
    };
    el.__fmLeave = function () {
      if (!S.sticky) goTo('default');
    };
    el.__fmBlur = function () {
      if (S.sticky) {
        S.sticky = false;
        goTo('default');
      }
  ...
  function forceButton(key, cls) {
    ...
    return e('button', {
      type: 'button',
      className: 'fm-force fm-force--' + key + (cls ? ' ' + cls : ''),
      'aria-pressed': active === key ? 'true' : 'false',
      ref: function (n) {
        btn.current[key] = n;
      },
      onMouseEnter: call('__fmSet', key),
      onFocus: call('__fmSet', key),
      onClick: call('__fmToggle', key)
    }, e('span', {
      className: 'fm-force__label'
    }, t.label), t.sub ? e('span', {
      className: 'fm-force__sub'
    }, t.sub) : null, e('span', {
      className: 'fm-force__q'
    }, t.q));
  }
  var line = function (id, extra) {
    return e('line', Object.assign({
      key: id,
      ref: function (n) {
        nodes.current[id] = n;
      }
  ...
  return e('div', {
    className: 'fm',
    ref: wrap,
    onMouseLeave: call('__fmLeave'),
    onBlur: call('__fmBlur')
  }, e('div', {
    className: 'fm__ends',
    ref: function (n) {
      nodes.current.ends = n;
    }
  }, e('span', {
    className: 'fm__end fm__end--from'
  }, 'Where', e('br'), 'you are'), e('span', {
    className: 'fm__end fm__end--to'
  }, 'Where you', e('br'), 'want to get to')), e('div', {
    className: 'fm__stage'
  }, e('svg', {
    className: 'fm__svg',
    ref: svg,
    viewBox: '0 0 1336 640',
    preserveAspectRatio: 'xMidYMid meet',
    'aria-hidden': 'true',
    focusable: 'false'
  }, line('ax_l', {
    stroke: 'rgba(23,25,25,0.45)',
    strokeWidth: 1
  }), line('ax_lhab', {
    stroke: FM_CORAL,
    strokeWidth: 2,
    ...
    return [-2, -1, 0, 1, 2].map(function (k) {
      return line('l_' + key + '_' + k, {
        stroke: FM_TEXT[key].c,
        strokeWidth: 1,
        opacity: 0
      });
    });
    ...
    ref: function (n) {
      nodes.current.you_ghost = n;
    },
    cx: 668,
    cy: 320,
    r: 64,
    fill: 'none',
    stroke: 'rgba(23,25,25,0.34)',
    strokeWidth: 1,
    strokeDasharray: '3 5',
    opacity: 0
    ...
    return e('path', {
      key: 'arc_' + k,
      ref: function (n) {
        nodes.current['arc_' + k] = n;
      },
      fill: 'none',
      stroke: k === 'push' || k === 'pull' ? FM_GREEN : FM_CORAL,
      strokeWidth: 2.5,
      opacity: 0
    });
  }), e('g', {
    ref: function (n) {
      nodes.current.you_g = n;
    }
  }, e('circle', {
    ref: function (n) {
      nodes.current.you_c = n;
    },
    cx: 668,
    cy: 320,
    r: 64,
    ...
    ref: function (n) {
      nodes.current.you_t = n;
    },
    x: 668,
    y: 325.5,
    textAnchor: 'middle',
    fill: '#F3F0E8',
    className: 'fm__youlabel'
  }, 'YOU'))), forceButton('push', 'fm-force--tl'), forceButton('pull', 'fm-force--tr'), forceButton('habit', 'fm-force--bl'), forceButton('anxiety', 'fm-force--br')),
  ...
  e('div', {
    ...
    return e('button', {
      type: 'button',
      className: 'fm__row fm__row--' + key,
      key: key,
      'aria-pressed': active === key ? 'true' : 'false',
      ref: function (n) {
        listRow.current[idx] = n;
      },
      onClick: call('__fmToggle', key)
    }, e('span', {
      className: 'fm__rowlabel'
    }, key === 'habit' ? 'Habit /' : t.label), e('span', {
      className: 'fm__rowq'
    }, t.q));
  })), e('div', {
    className: 'wm-where',
    ref: function (n) {
      nodes.current.where = n;
    }
  }, e('h3', {
    ...
    return e('li', {
      ...
      key: iv.name,
      ref: function (n) {
        (nodes.current.chips = nodes.current.chips || [])[idx] = n;
      }
    }, iv.name);
  })), e('p', {
...
function WmHow() {
  ...
  }, 'Four forces decide whether a change actually happens. Two push you towards it: the situation becoming harder to tolerate, and something better you want instead. Two hold you in place: what the current situation still does for you, and what feels risky about changing. We map all four before deciding what to work on.'), e(WmForcesMap, null)));
}
function WmFit() {
  ...
  function column(heading, items, mark, no) {
    return e('div', null, e('h3', {
      ...
      className: 'wm-fit__rule' + (no ? ' wm-fit__rule--no' : ''),
      'aria-hidden': 'true'
    }), e('ul', {
