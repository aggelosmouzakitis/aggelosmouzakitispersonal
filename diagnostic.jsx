...
function getLevelVisual(score) {
  ...
  if (score <= 2.5) return { color: '#10bb82', pct };
...
function DiagnosticPage() {
  ...
  const C = {
    ...
    progressFill: (pct) => ({ height: '1px', background: '#10bb82', width: pct + '%', transition: 'width .2s ease' }),
    sectionRow: {
  ...
  if (screen === 'question') return (
    <div style={C.page} ref={mainRef}>
      ...
      <h2 style={{ fontSize: '20px', lineHeight: 1.85, marginBottom: '1.8rem', fontWeight: 400, color: '#282726' }}>
        {currentQ.text}
      </h2>
  ...
  if (screen === 'results' && results) return (
    <div style={C.page} ref={mainRef}>
      ...
      <p style={C.p}>Your score: <span style={{ color: '#10bb82' }}>{fmt(results.overall)}</span> / 5.00</p>
      ...
      {results.nextStep && (
        <div style={{ borderLeft: '2px solid #10bb82', padding: '2px 0 2px 14px', margin: '1.6rem 0' }}>
      ...
      <div style={{ border: '1px solid rgba(16,187,130,0.3)', background: 'rgba(16,187,130,0.04)', padding: '1.4rem', marginTop: '2.5rem' }}>
