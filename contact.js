...
var INTERESTS = [{
  value: 'orientation',
  label: 'Free 30-minute orientation chat'
}, {
  value: 'experience-to-offer',
  label: 'Experience-to-Offer Audit'
}, {
  value: 'solo-business-growth',
  label: 'Solo Business Growth Audit'
}, {
  value: 'private-sparring',
  label: 'Private Sparring'
}];
var VALID_INTEREST = INTERESTS.reduce(function (m, o) {
  m[o.value] = o.label;
  return m;
}, {});
...
.ct-main{background:#F3F0E8;color:#3A403A;font-family:var(--font-body)}
...
.ct-h1{margin:14px 0 0;font-family:var(--font-heading);font-synthesis:none;font-size:clamp(34px,4.4vw,56px);font-weight:800;line-height:1.02;letter-spacing:-0.04em;color:#171919;text-wrap:balance}
.ct-intro{margin:16px 0 0;max-width:60ch;font-size:18px;line-height:1.6;color:#3A403A}
.ct-grid{display:grid;grid-template-columns:minmax(0,65fr) minmax(0,35fr);gap:32px;align-items:start}
.ct-card{background:#F3F0E8;border:1px solid rgba(23,25,25,0.12);border-radius:14px;padding:clamp(24px,3vw,40px)}
.ct-social{background:#16231E;color:#F3F0E8;border-radius:14px;padding:clamp(24px,3vw,36px)}

.ct-field{margin-bottom:20px}
.ct-label{display:block;font-size:14px;font-weight:700;letter-spacing:0.02em;color:#171919;margin-bottom:8px}
.ct-req{color:#047857}
.ct-input,.ct-select,.ct-textarea{display:block;width:100%;min-height:52px;border:1px solid rgba(23,25,25,0.22);border-radius:10px;background:#F3F0E8;color:#3A403A;font-family:inherit;font-size:16px;line-height:1.5;padding:14px 15px;outline:none;transition:border-color .16s,box-shadow .16s}
...
.ct-help{margin:7px 0 0;font-size:13px;line-height:1.5;color:#6A6F67}
.ct-count{margin:7px 0 0;font-size:13px;color:#6A6F67;text-align:right}
.ct-err{margin:7px 0 0;font-size:13px;line-height:1.5;color:#c0392b;font-weight:600}
.ct-hp{position:absolute;left:-9999px;width:1px;height:1px;overflow:hidden}

.ct-submit{width:100%;min-height:56px;display:inline-flex;align-items:center;justify-content:center;gap:9px;background:#047857;color:#F3F0E8;border:1.5px solid #047857;border-radius:2px;font-family:inherit;font-weight:700;font-size:14px;letter-spacing:0.06em;text-transform:uppercase;cursor:pointer;transition:filter .18s}
.ct-submit:hover{background:#03654A}
...
.ct-micro{margin:14px 0 0;font-size:14px;line-height:1.6;color:#6A6F67}
.ct-privacy{margin:10px 0 0;font-size:13px;line-height:1.6;color:#6A6F67}
...
.ct-success h2{margin:0 0 12px;font-family:var(--font-heading);font-synthesis:none;font-size:clamp(26px,3vw,36px);font-weight:800;line-height:1.1;letter-spacing:-0.03em;color:#171919;outline:none}
.ct-success p{margin:0 0 24px;font-size:18px;line-height:1.6;color:#3A403A;max-width:52ch}
.ct-success__row{display:flex;flex-wrap:wrap;gap:12px}

.ct-social h2{margin:0 0 12px;font-family:var(--font-heading);font-size:clamp(22px,2.4vw,28px);font-weight:800;line-height:1.15;letter-spacing:-0.02em;color:#F3F0E8}
.ct-social p{margin:0 0 24px;font-size:16px;line-height:1.6;color:#C0C9BF}
.ct-social__btns{display:flex;flex-direction:column;gap:12px}
.ct-sbtn{display:inline-flex;align-items:center;justify-content:center;gap:8px;width:100%;min-height:52px;padding:0 12px;border:1.5px solid rgba(243,240,232,0.5);border-radius:2px;background:transparent;color:#F3F0E8;font-family:inherit;font-weight:700;font-size:12px;letter-spacing:0.02em;text-transform:uppercase;text-decoration:none;transition:background .18s,border-color .18s,color .18s}
.ct-sbtn:hover{background:#047857;border-color:#047857;color:#F3F0E8}
...
.ct-btn2--green{background:#047857;color:#F3F0E8;border:1.5px solid #047857}
.ct-btn2--green:hover{background:#03654A}
.ct-btn2--outline{background:transparent;color:#3A403A;border:1.5px solid rgba(23,25,25,.35)}
...
function SocialIcon(name) {
  return window.BrandIcon ? e(window.BrandIcon, {
    name: name,
    size: 18
  }) : null;
}
function ContactForm() {
  ...
  R.useEffect(function () {
    track('contact_page_viewed', {
      source_page: typeof document !== 'undefined' && document.referrer || 'direct'
    });
    if (preselected) track('interest_preselected', {
  ...
  function focusFirstInvalid(er) {
    ...
    for (var i = 0; i < order.length; i++) {
      if (er[order[i]] && refs[order[i]].current) {
  ...
  function onSubmit(ev) {
    ...
    var body = ['New website enquiry', '', 'Full name: ' + sanitize(name), 'Email: ' + sanitize(email), 'Interested in: ' + interestLabel, '', 'Message:', sanitize(msg).slice(0, MAXLEN), '', 'Source page: ' + source, 'Submitted: ' + when.toISOString() + ' (' + when.toString() + ')'].join('\n');
    var payload = {
    ...
    function ok() {
      ...
      safeSet('ct_last', String(lastSubmitRef.current));
    ...
    function fail(e2) {
      try {
        console.error('Contact send error:', e2);
      } catch (e3) {}
    ...
    if (typeof window !== 'undefined' && window.emailjs) {
      var settled = false;
      emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, payload, EMAILJS_PUBLIC_KEY).then(function () {
        if (!settled) {
          settled = true;
          ok();
        }
      ...
      fail(new Error('EmailJS unavailable'));
    }
  }
  function setFormErr(v) {
    formErrS[1](v);
  }
  function safeGet(k) {
    try {
  ...
  if (status === 'success') {
    return e('div', {
      ...
      tabIndex: -1,
      ref: successRef
    }, 'Thank you. I’ve got your message.'), e('p', null, 'I’ll read it myself and get back to you personally.'), e('div', {
      className: 'ct-success__row'
    }, e('a', {
      className: 'ct-btn2 ct-btn2--green',
      href: '/'
    }, 'Back to the homepage →'), e('a', {
      className: 'ct-btn2 ct-btn2--outline',
      href: LINKEDIN,
      target: '_blank',
      rel: 'noopener noreferrer',
      onClick: function () {
        track('linkedin_cta_clicked', {
          context: 'contact-success'
        });
      }
    }, 'Connect on LinkedIn →')));
  }
  ...
  e('div', {
    ...
    className: 'ct-req'
  }, '*')), e('input', Object.assign(field('name'), {
    className: 'ct-input',
    type: 'text',
  ...
  e('div', {
    ...
    className: 'ct-req'
  }, '*')), e('input', Object.assign(field('email'), {
    className: 'ct-input',
    type: 'email',
  ...
  e('div', {
    ...
    className: 'ct-label',
    htmlFor: 'interest'
  }, 'What are you interested in? ', e('span', {
    className: 'ct-req'
  }, '*')), e('select', Object.assign(field('interest', null, {
    helpId: 'interest-help'
  }), {
    className: 'ct-select',
    value: interest,
    onChange: function (ev) {
      interestS[1](ev.target.value);
    ...
    value: ''
  }, 'Choose the closest option'), INTERESTS.map(function (o) {
    return e('option', {
      key: o.value,
      value: o.value
    }, o.label);
    ...
    id: 'interest-err',
    className: 'ct-err'
  }, errors.interest) : null, e('p', {
    id: 'interest-help',
    className: 'ct-help'
  }, 'Choose the orientation chat if you are not sure where to begin.')),
  // Message
  e('div', {
    className: 'ct-field'
  }, e('label', {
    className: 'ct-label',
    htmlFor: 'message'
  }, 'What would you like me to know? ', e('span', {
    className: 'ct-req'
  }, '*')), e('textarea', Object.assign(field('message', null, {
    helpId: 'message-help'
  }), {
    className: 'ct-textarea',
    placeholder: 'What’s going on, and what would you like some help with?',
    maxLength: MAXLEN,
    value: msg,
    onChange: function (ev) {
      msgS[1](ev.target.value);
    }
  })), errors.message ? e('p', {
    id: 'message-err',
    className: 'ct-err'
  }, errors.message) : null, e('p', {
    id: 'message-help',
    className: 'ct-help'
  }, 'A few lines are enough.'), showCount ? e('p', {
    className: 'ct-count',
    'aria-live': 'polite'
  }, msg.length + ' / ' + MAXLEN) : null), e('button', {
    className: 'ct-submit',
    type: 'submit',
    disabled: sending
  }, sending ? 'Sending…' : 'Send my message →'), e('p', {
    className: 'ct-micro'
  }, 'I read every message myself. You’ll hear back from me personally.'), e('p', {
    className: 'ct-privacy'
  }, 'Your details will only be used to reply to this message. ', e('a', {
    href: PRIVACY_HREF
  }, 'Privacy policy'), '.'));
}
function SocialPanel() {
  return e('aside', {
    className: 'ct-social'
  }, e('h2', null, 'Prefer a DM?'), e('p', null, 'You can also message me where you already spend time. Use whichever feels easier. I read those messages myself too.'), e('div', {
    className: 'ct-social__btns'
  }, e('a', {
    className: 'ct-sbtn',
    href: LINKEDIN,
    target: '_blank',
    ...
    onClick: function () {
      track('linkedin_cta_clicked', {
        context: 'contact-panel'
      });
    }
  }, SocialIcon('LinkedIn'), e('span', null, 'Message me on LinkedIn →')), e('a', {
    className: 'ct-sbtn',
    href: INSTAGRAM,
    target: '_blank',
    rel: 'noopener noreferrer',
    onClick: function () {
      track('instagram_cta_clicked', {
        context: 'contact-panel'
      });
    }
...
function ContactPage() {
  return e(React.Fragment, null, e(window.ChromeStyles), e(ContactStyles), e(window.SiteHeader, {
    page: 'contact',
    lang: 'en'
  }), e('main', {
    className: 'ct-main'
    ...
    className: 'ct-h1'
  }, 'Tell me what’s going on.'), e('p', {
    className: 'ct-intro'
  }, 'You do not need to have it fully worked out or explain it perfectly. A few honest lines are enough for me to understand where to start.')), e('div', {
    className: 'ct-grid'
  }, e('div', {
    className: 'ct-card'
  }, e(ContactForm, null)), e(SocialPanel, null)))), e(window.SiteFooterX, {
    lang: 'en'
  }));
}
