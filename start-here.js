...
var Q1 = {
  ...
  cards: [{
    ...
    id: 'tangled',
    label: "It's all tangled together",
...
var FOLLOW_UPS = {
  business: {
    ...
    options: [{
      text: "It's running, and I want it to grow without grinding myself down.",
      direction: 'business',
      ...
      text: "I'm building it on the side while I still have a job.",
      direction: 'career',
      ...
      text: 'I keep not doing the things I already know would move it.',
      direction: 'personal',
  ...
  career: {
    ...
    instruction: 'Still the closest one.',
    options: [{
      text: 'I want to leave employment and build something of my own.',
      direction: 'career',
      ...
      text: "I'm weighing a specific move and keep going back and forth.",
      direction: 'personal',
      ...
      text: 'I already work for myself and the real problem is the business.',
      direction: 'business',
  ...
  tangled: {
    ...
    options: [{
      ...
      text: 'Start from the career decision.',
      direction: 'career',
...
function StartHereStyles() {
  ...
  '.sh-grid{display:grid;grid-template-columns:1fr 1fr;gap:20px}', '.sh-card{position:relative;display:flex;flex-direction:column;min-height:176px;text-align:left;background:#F3F0E8;border:1.5px solid rgba(23,25,25,.16);border-radius:8px;padding:24px 28px 26px;cursor:pointer;font-family:inherit;color:#171919;box-shadow:5px 5px 0 rgba(4,120,87,.18);transition:transform .13s ease,box-shadow .13s ease,border-color .13s ease}', '.sh-card:hover{border-color:rgba(23,25,25,.30);transform:translate(-2px,-2px);box-shadow:7px 7px 0 rgba(4,120,87,.26)}', '.sh-card:focus-visible{outline:2px solid #047857;outline-offset:3px}', '.sh-card.is-sel{border-color:#047857;box-shadow:5px 5px 0 rgba(4,120,87,.34)}', '.sh-card.is-pressing{border-color:#047857;transform:translate(5px,5px);box-shadow:0 0 0 rgba(4,120,87,0)}', '.sh-card__top{display:flex;align-items:center;justify-content:space-between;margin-bottom:clamp(20px,2.6vw,30px)}', '.sh-card__num{font-family:var(--font-heading);font-size:17px;font-weight:800;letter-spacing:.02em;color:#171919;font-variant-numeric:tabular-nums}', '.sh-card.is-sel .sh-card__num,.sh-card.is-pressing .sh-card__num{color:#047857}', '.sh-card__arrow{font-size:20px;line-height:1;color:#171919;transition:transform .15s,color .15s}', '.sh-card:hover .sh-card__arrow{transform:translateX(3px)}', '.sh-card.is-sel .sh-card__arrow,.sh-card.is-pressing .sh-card__arrow{color:#047857}', '.sh-card__label{display:block;font-family:var(--font-heading);font-synthesis:none;font-size:clamp(19px,1.5vw,23px);font-weight:800;line-height:1.08;letter-spacing:.015em;text-transform:uppercase;color:#171919}', '.sh-card.is-sel .sh-card__label,.sh-card.is-pressing .sh-card__label{color:#047857}', '.sh-card__copy{display:block;margin-top:12px;font-size:16px;line-height:1.5;font-weight:400;letter-spacing:0;color:#3A403A;text-wrap:pretty}', '.sh-card--plain .sh-card__copy{margin-top:0;font-size:clamp(19px,1.5vw,23px);line-height:1.3;font-weight:500;letter-spacing:-.01em;color:#171919}',
  // outcome layout (result screen)
  '.sh-outcome{display:grid;grid-template-columns:7fr 5fr;gap:44px;align-items:start}', '.sh-outcome__lead{min-width:0}', '.sh-outcome__body{font-size:18px;line-height:1.6;color:#3A403A;margin:0;max-width:60ch}', '.sh-outcome__kind{font-size:14px;line-height:1.5;color:#6A6F67;margin:0 0 20px}', '.sh-panel{background:#F3F0E8;border:1.5px solid rgba(23,25,25,.20);border-radius:8px;padding:26px 24px}', '.sh-btn{display:inline-flex;align-items:center;justify-content:center;width:100%;min-height:54px;padding:0 20px;background:#047857;color:#F3F0E8;font-family:inherit;font-weight:700;font-size:13px;letter-spacing:.05em;text-transform:uppercase;line-height:1.25;text-align:center;border:1.5px solid #047857;border-radius:0;text-decoration:none;cursor:pointer;transition:filter .16s}', '.sh-btn:hover{background:#03654A}', '.sh-btn:focus-visible{outline:2px solid #047857;outline-offset:3px}', '.sh-btn--ghost{background:transparent;color:#171919;border-color:rgba(23,25,25,.30)}', '.sh-btn--ghost:hover{filter:none;background:transparent;color:#171919;border-color:#171919}', '.sh-btn + .sh-btn{margin-top:12px}', '.sh-note{font-size:13px;line-height:1.5;color:#6A6F67;margin:10px 0 0}', '.sh-panel__group + .sh-panel__group{margin-top:16px;padding-top:16px;border-top:1px solid rgba(23,25,25,.12)}',
  ...
  // responsive
  '@media (max-width:820px){.sh-outcome{grid-template-columns:1fr;gap:28px}}', '@media (max-width:680px){.sh-grid{grid-template-columns:1fr;gap:18px}.sh-card{min-height:0;padding:22px 24px 24px}.sh-card__top{margin-bottom:18px}.sh-stage{min-height:0}}', '@media (prefers-reduced-motion:reduce){.sh-card,.sh-card__arrow,.sh-btn,.sh-quiet{transition:none}.sh-card:hover,.sh-card.is-pressing{transform:none}}'].join('');
  return e('style', {
...
function StartHerePage() {
  ...
  function markChoice(i) {
    var upd = {
      screen: stRef.current.screen,
      situation: stRef.current.situation,
      choice: i
    };
    try {
  ...
  function fallback(key, text, href, low) {
    return e('a', {
      key: key,
      className: 'sh-textlink' + (low ? ' sh-textlink--low' : ''),
      href: href,
      onClick: function () {
  ...
  function card(i, opts) {
    ...
    return e('button', {
      ...
      className: 'sh-card__label'
    }, opts.label) : null, e('span', {
  ...
  function followUpScreen() {
    ...
    return e('div', {
      ...
      className: 'sh-fallbacks'
    }, fallback('free-tools', 'Would rather work it out on your own first? Explore the free tools →', FREE_TOOLS)), controls(false));
  }
  ...
  function resultScreen() {
    ...
    return e('div', {
      ...
      href: d.href,
      onClick: function () {
        track('start_here_direction', {
          direction: st.direction
        });
      }
    }, 'SEE HOW THIS WORKS →'), e('p', {
      className: 'sh-note'
    }, 'What it is, how it runs and what it costs — before you decide anything.')), e('div', {
      className: 'sh-panel__group'
    }, e('a', {
      className: 'sh-btn sh-btn--ghost',
      href: d.tool.href,
      onClick: function () {
        track('start_here_tool', {
          tool: d.tool.href
        });
      }
    }, 'TRY A FREE TOOL FIRST →'), e('p', {
      className: 'sh-note'
    }, d.tool.name), e('p', {
      className: 'sh-note'
    }, fallback('ask', 'Rather just ask me something? Ask me anything →', ASK, true))))), controls(false));
  }
  var body;
  if (st.screen === 'q2') body = followUpScreen();else if (st.screen === 'result') body = resultScreen();else body = situationScreen();
