...
function StartHereStyles() {
  var css = ['.sh-root{display:flex;flex-direction:column;min-height:100vh;background:#F4F1EA}', '.sh-main{flex:1 0 auto;background:#F4F1EA;color:#282726}', '.sh-wrap{width:min(100% - 40px,1000px);margin-inline:auto;padding-block:clamp(40px,6vw,72px) clamp(48px,7vw,88px)}', '.sh-stage{min-height:clamp(440px,56vh,560px)}', '.sh-intro{max-width:760px}', '.sh-step{font-size:12px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:#059669;margin:0 0 14px}', '.sh-h1{font-family:var(--font-heading);font-synthesis:none;font-size:clamp(30px,4.4vw,46px);font-weight:800;line-height:1.04;letter-spacing:-.03em;color:#181A1C;margin:0 0 12px;outline:none}', '.sh-instr{font-size:16px;line-height:1.55;color:#282726;margin:0 0 30px}',
  // card grid
  '.sh-grid{display:grid;grid-template-columns:1fr 1fr;gap:18px}', '.sh-card{position:relative;display:flex;flex-direction:column;justify-content:space-between;gap:26px;min-height:196px;text-align:left;background:#FFFFFF;border:1.5px solid rgba(24,26,28,.20);border-radius:6px;padding:22px 24px;cursor:pointer;font-family:inherit;color:#181A1C;transition:border-color .15s,box-shadow .15s,transform .15s}', '.sh-card:hover{border-color:rgba(24,26,28,.5)}', '.sh-card:focus-visible{outline:2px solid #059669;outline-offset:3px}', '.sh-card.is-sel{border-color:#059669;box-shadow:6px 6px 0 rgba(5,150,105,.20);transform:translate(-1px,-1px)}', '.sh-card__top{display:flex;align-items:center;justify-content:space-between}', '.sh-card__num{font-family:var(--font-heading);font-size:15px;font-weight:800;letter-spacing:.02em;color:#181A1C;font-variant-numeric:tabular-nums}', '.sh-card.is-sel .sh-card__num{color:#059669}', '.sh-card__arrow{font-size:20px;line-height:1;color:#181A1C;transition:transform .15s,color .15s}', '.sh-card:hover .sh-card__arrow{transform:translateX(3px)}', '.sh-card.is-sel .sh-card__arrow{color:#059669}', '.sh-card__body{display:block}', '.sh-card__copy{font-size:18px;line-height:1.34;font-weight:500;color:#181A1C}', '.sh-card__kicker{display:block;margin-top:10px;font-size:12px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#059669}',
  // outcome
  '.sh-outcome{display:grid;grid-template-columns:7fr 5fr;gap:44px;align-items:start}', '.sh-outcome__lead{min-width:0}', '.sh-outcome__h{font-family:var(--font-heading);font-synthesis:none;font-size:clamp(30px,4.4vw,46px);font-weight:800;line-height:1.05;letter-spacing:-.03em;color:#181A1C;margin:0 0 18px;outline:none}', '.sh-outcome__body{font-size:18px;line-height:1.6;color:#282726;margin:0;max-width:60ch}', '.sh-panel{background:#FFFFFF;border:1.5px solid rgba(24,26,28,.20);border-radius:8px;padding:26px 24px}', '.sh-btn{display:inline-flex;align-items:center;justify-content:center;width:100%;min-height:54px;padding:0 20px;background:#059669;color:#fff;font-family:inherit;font-weight:700;font-size:13px;letter-spacing:.05em;text-transform:uppercase;line-height:1.25;text-align:center;border:1.5px solid #059669;border-radius:0;text-decoration:none;cursor:pointer;transition:filter .16s}', '.sh-btn:hover{filter:brightness(.93)}', '.sh-btn--ghost{background:transparent;color:#181A1C;border-color:rgba(24,26,28,.30)}', '.sh-btn--ghost:hover{filter:none;border-color:#181A1C}', '.sh-btn + .sh-btn{margin-top:12px}', '.sh-note{font-size:13px;line-height:1.5;color:#5E6264;margin:10px 0 0}', '.sh-panel__group + .sh-panel__group{margin-top:16px;padding-top:16px;border-top:1px solid rgba(24,26,28,.12)}',
  // quiet controls
  '.sh-controls{display:flex;flex-wrap:wrap;gap:24px;align-items:center;margin-top:36px}', '.sh-quiet{background:none;border:0;padding:6px 0;font-family:inherit;font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#8A8A8A;cursor:pointer;transition:color .15s}', '.sh-quiet:hover{color:#181A1C}', '.sh-quiet:focus-visible{outline:2px solid #059669;outline-offset:3px}',
  // responsive
  '@media (max-width:820px){.sh-outcome{grid-template-columns:1fr;gap:28px}}', '@media (max-width:680px){.sh-grid{grid-template-columns:1fr}.sh-card{min-height:0;gap:20px}.sh-stage{min-height:0}}', '@media (prefers-reduced-motion:reduce){.sh-card,.sh-card__arrow,.sh-btn,.sh-quiet{transition:none}.sh-card.is-sel{transform:none}}'].join('');
  return e('style', {
