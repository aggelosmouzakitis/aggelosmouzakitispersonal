...
function StartHereStyles() {
  ...
  '.sh-card__top{display:flex;align-items:center;justify-content:space-between;margin-bottom:clamp(26px,3.2vw,40px)}', '.sh-card__num{font-family:var(--font-heading);font-size:17px;font-weight:800;letter-spacing:.02em;color:#181A1C;font-variant-numeric:tabular-nums}', '.sh-card.is-sel .sh-card__num,.sh-card.is-pressing .sh-card__num{color:#047857}', '.sh-card__arrow{font-size:20px;line-height:1;color:#181A1C;transition:transform .15s,color .15s}', '.sh-card:hover .sh-card__arrow{transform:translateX(3px)}', '.sh-card.is-sel .sh-card__arrow,.sh-card.is-pressing .sh-card__arrow{color:#047857}', '.sh-card__body{display:block}', '.sh-card__copy{font-size:clamp(20px,1.55vw,24px);line-height:1.28;font-weight:500;letter-spacing:-.01em;color:#181A1C}', '.sh-card__kicker{display:block;margin-top:auto;padding-top:18px;font-size:13px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#047857}',
  // outcome
  ...
  // responsive
  '@media (max-width:820px){.sh-outcome{grid-template-columns:1fr;gap:28px}}', '@media (max-width:680px){.sh-grid{grid-template-columns:1fr;gap:18px}.sh-card{min-height:0;padding:22px 24px 24px}.sh-card__top{margin-bottom:22px}.sh-stage{min-height:0}}', '@media (prefers-reduced-motion:reduce){.sh-card,.sh-card__arrow,.sh-btn,.sh-quiet{transition:none}.sh-card:hover,.sh-card.is-pressing{transform:none}}'].join('');
  return e('style', {
