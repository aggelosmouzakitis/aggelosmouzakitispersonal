...
function ftSubmitInterest(tool, email, done) {
  var now = new Date();
  var detail = ['Tool: ' + tool.title, 'Tool id: ' + tool.id, 'Source: ' + (typeof window !== 'undefined' ? window.location.pathname : '/free-tools/'), 'Requested: ' + now.toISOString()].join('\n');
  ...
  if (typeof window !== 'undefined' && window.emailjs) {
    try {
      emailjs.send(FT_EMAILJS_SERVICE, FT_EMAILJS_TEMPLATE, payload).then(function () {
        finish(true);
      }).catch(function (err) {
        console.error('Free tools interest error:', err);
        finish(true);
      });
...
function artConstraint() {
  return ftArt([ftPath('a', 'M0 8 H104 L150 44'), ftPath('b', 'M0 30 H104 L150 48'), ftPath('c', 'M0 52 H150'), ftPath('d', 'M0 74 H104 L150 56'), ftPath('e', 'M0 96 H104 L150 60'), ftRect('f', 158, 30, 4, 44, {
    fill: FT_GREEN
...
function artQuit() {
  return ftArt([ftPath('a', 'M0 52 H130', FT_GREEN, 1.75), ftPath('b', 'M130 52 L262 12'), ftPath('c', 'M130 52 H262'), ftPath('d', 'M130 52 L262 92'), ftCircle('e', 130, 52, 5, {
    fill: FT_GREEN
  })]);
}
...
function artBurnout() {
  var bars = [[4, 12, 80, FT_GREEN], [36, 20, 72, 'rgba(23,25,25,0.62)'], [68, 30, 62, 'rgba(23,25,25,0.52)'], [100, 42, 50, 'rgba(23,25,25,0.44)'], [132, 54, 38, 'rgba(23,25,25,0.36)'], [164, 64, 28, 'rgba(23,25,25,0.28)'], [196, 74, 18, 'rgba(23,25,25,0.22)'], [228, 82, 10, 'rgba(23,25,25,0.16)'], [260, 88, 4, 'rgba(23,25,25,0.12)']];
  return ftArt(bars.map(function (b, i) {
    return ftRect('b' + i, b[0], b[1], 4, b[2], {
      fill: b[3]
    });
  }));
}
...
function artSales() {
  return ftArt([ftCircle('a', 24, 52, 15, {
    stroke: FT_INK2,
    sw: 1.6
  }), ftCircle('b', 264, 52, 15, {
    fill: FT_INK2
  }), ftPath('c', 'M44 52 H126'), ftPath('d', 'M170 52 H244'), ftPath('e', 'M140 34 V70 M158 34 V70', FT_INK2, 1.6)]);
}
// 08 — two weights either side of a price line.
function artPricing() {
  return ftArt([ftCircle('a', 74, 52, 34, {
    stroke: FT_INK2,
    sw: 1.6
  }), ftCircle('b', 222, 52, 20, {
    fill: FT_INK2
  }), ftPath('c', 'M160 14 V90'), ftPath('d', 'M114 52 H150 M170 52 H198')]);
}
// 09 — one decision, two futures of different weight.
function artJob() {
  return ftArt([ftPath('a', 'M14 52 L240 18'), ftPath('b', 'M14 52 L240 88', FT_INK2, 1.6), ftCircle('c', 14, 52, 4, {
    fill: FT_INK2
  }), ftCircle('d', 252, 16, 6, {
    stroke: FT_INK2,
    sw: 1.6
  }), ftCircle('e', 252, 90, 12, {
    fill: FT_INK2
  })]);
}
// 10 — a straight line detouring around the thing in the middle.
function artAvoiding() {
  return ftArt([ftCircle('a', 150, 58, 22, {
    fill: FT_INK2
...
function ftCount(cat) {
  return cat === 'all' ? FT_TOOLS.length : FT_TOOLS.filter(function (t) {
    return t.cat === cat;
  }).length;
}
...
var FREE_TOOLS_CSS = ['.ft-main{background:var(--bone,#F3F0E8);color:var(--ink,#171919)}',
// hero
'.ft-hero{position:relative;background:var(--bone,#F3F0E8);overflow:clip}', '.ft-hero__rule{position:absolute;top:0;bottom:0;left:66%;width:1px;background:rgba(4,120,87,0.55)}', '.ft-hero__in{position:relative;display:flex;flex-wrap:wrap;align-items:center;gap:clamp(40px,5vw,64px);padding-block:clamp(72px,9vw,104px) clamp(64px,8vw,96px)}', '.ft-hero__copy{flex:1 1 380px;min-width:0;max-width:760px}', '.ft-eyebrow{margin:0;color:var(--green,#047857);font-size:13px;font-weight:700;line-height:1.3;letter-spacing:0.11em;text-transform:uppercase}', '.ft-hero__h1{max-width:22ch;margin:18px 0 0;font-family:var(--font-heading);font-synthesis:none;font-size:clamp(38px,5vw,68px);font-weight:800;line-height:0.98;letter-spacing:-0.048em;color:var(--heading-ink,#14201C);text-wrap:balance}', '.ft-hero__tick{width:96px;height:1px;background:var(--green,#047857);margin:30px 0 0}', '.ft-hero__lead{max-width:56ch;margin:26px 0 0;font-size:21px;line-height:1.5;color:var(--ink-2,#3A403A);text-wrap:pretty}', '.ft-hero__note{max-width:48ch;margin:16px 0 0;font-size:15px;line-height:1.5;color:var(--meta,#6A6F67)}', '.ft-hero__fig{flex:0 1 clamp(190px,26vw,400px);margin:0;position:relative;aspect-ratio:1;align-self:center}', '.ft-hero__disc,.ft-hero__disc-cut{position:absolute;inset:8% 6% 6% 8%;border-radius:50%}', '.ft-hero__disc{background:var(--green,#047857)}', '.ft-hero__disc-cut{background:linear-gradient(90deg,rgba(243,240,232,0) 58%,var(--bone,#F3F0E8) 58%)}', '.ft-hero__h{position:absolute;left:0;right:-14%;top:50%;height:1px;background:rgba(23,25,25,0.24)}', '.ft-hero__v{position:absolute;left:58%;top:6%;bottom:4%;width:1px;background:rgba(4,120,87,0.55)}',
// collection band
...
'.ft-collection{background:var(--bone-deep,#EDE8DB);padding-block:clamp(64px,8vw,96px) clamp(72px,9vw,112px);--ft-green-sm:var(--green-pressed,#03654A);--ft-meta:#60655D}', '.ft-collection__head{display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:16px 40px}', '.ft-collection__h{margin:0;max-width:20ch;font-family:var(--font-heading);font-synthesis:none;font-size:clamp(30px,3.7vw,46px);font-weight:800;line-height:1.02;letter-spacing:-0.042em;color:var(--heading-ink,#14201C);text-wrap:balance}',
// filter
...
'.ft-filter__btn{display:inline-flex;align-items:baseline;gap:9px;min-height:44px;padding:20px 0 16px;background:none;border:0;border-bottom:2px solid transparent;margin-bottom:-1px;cursor:pointer;font-family:inherit;font-size:14px;font-weight:700;line-height:1;letter-spacing:0.11em;text-transform:uppercase;color:var(--ink-2,#3A403A);transition:color .16s,border-color .16s}', '.ft-filter__btn:hover{color:var(--ft-green-sm,#03654A)}', '.ft-filter__btn[aria-pressed="true"]{color:var(--ft-green-sm,#03654A);border-bottom-color:var(--green,#047857)}', '.ft-filter__count{font-size:11.5px;font-weight:700;letter-spacing:0.06em;color:var(--ft-meta,#60655D);font-variant-numeric:tabular-nums}', '.ft-filter__btn[aria-pressed="true"] .ft-filter__count{color:var(--ft-green-sm,#03654A)}',
// grid + cards
...
function ComingSoonSignup(props) {
  var tool = props.tool;
  var R = React;
  var stageState = R.useState('closed');
  var stage = stageState[0],
    setStage = stageState[1];
  var valueState = R.useState('');
  var value = valueState[0],
    setValue = valueState[1];
  var errorState = R.useState('');
  ...
  function open() {
    setStage('open');
    ftTrack('free_tool_interest_open', {
      tool: tool.id
    });
  }
  function submit(ev) {
    ...
    setError('');
    setStage('sending');
    ftTrack('free_tool_interest_submit', {
      tool: tool.id
    });
    ftSubmitInterest(tool, email, function () {
      setStage('done');
    });
  }
  if (stage === 'done') {
    return e('div', {
      ...
      className: 'ft-done__h'
    }, "You're on the list."), e('p', {
      className: 'ft-done__p'
    }, 'One email, about this tool only.'));
  }
  if (stage === 'closed') {
    return e('button', {
      type: 'button',
  ...
  return e('form', {
    ...
    className: 'ft-form__label',
    htmlFor: inputId
  }, 'Email address'), e('p', {
    className: 'ft-form__hint'
  }, "I'll tell you when this one specifically is ready."), e('div', {
    className: 'ft-form__row'
  }, e('input', {
    id: inputId,
    ref: inputRef,
    ...
    type: 'submit',
    className: 'ft-form__submit',
    disabled: sending
  }, sending ? 'Sending…' : 'Let me know', e('span', {
    'aria-hidden': 'true'
  }, sending ? '' : '→'))), error ? e('p', {
    className: 'ft-form__error',
    id: errorId,
    role: 'alert'
  }, error) : null);
}
...
function ToolCard(props) {
  ...
  var head = e('div', {
    ...
    className: 'ft-card__num',
    'aria-hidden': 'true'
  }, num), e('span', {
    className: 'ft-card__kind'
  }, label), t.soon ? e('span', {
    className: 'ft-card__soon'
  }, 'Coming soon') : null);
  var art = e('div', {
    className: 'ft-card__figure'
  }, t.art());
  var body = [e('h3', {
    className: 'ft-card__h',
    key: 'h'
  }, t.title), e('p', {
    className: 'ft-card__desc',
    key: 'd'
  }, t.desc), t.note ? e('p', {
    className: 'ft-card__note',
    key: 'n'
  }, t.note) : null];
  if (t.soon) {
    return e('div', {
      className: 'ft-card ft-card--soon'
    }, head, art, body, e(ComingSoonSignup, {
      tool: t
    }));
  }
  return e('a', {
  ...
  }, head, art, body, e('span', {
    className: 'ft-card__go'
  }, t.meta, ' ', e('span', {
    'aria-hidden': 'true'
  }, '→')));
}
...
function FreeToolsPage() {
  var R = React;
  var catState = R.useState('all');
  var cat = catState[0],
    setCat = catState[1];
  var liveRef = R.useRef(null);
  var firstRender = R.useRef(true);
  var visible = FT_TOOLS.filter(function (t) {
  ...
  R.useEffect(function () {
    ...
    if (liveRef.current) {
      ...
      liveRef.current.textContent = 'Showing ' + n + ' ' + (n === 1 ? 'tool' : 'tools') + (cat === 'all' ? '.' : ' in ' + ftCatLabel(cat) + '.');
    }
  }, [cat]);
  function choose(next) {
    if (next === cat) return;
    setCat(next);
    ftTrack('free_tools_filter', {
      category: next
    });
  }
  var filter = e('div', {
    className: 'ft-filter',
    role: 'group',
    'aria-label': 'Filter tools by subject'
  }, FT_CATEGORIES.map(function (c) {
    return e('button', {
      key: c.id,
      type: 'button',
      className: 'ft-filter__btn',
      'aria-pressed': cat === c.id ? 'true' : 'false',
      onClick: function () {
        choose(c.id);
      }
    }, e('span', null, c.label), e('span', {
      className: 'ft-filter__count'
    }, String(ftCount(c.id))));
  }));
  var grid = e('div', {
    className: 'ft-grid'
  ...
  return e(React.Fragment, null, e(window.ChromeStyles), e(FreeToolsStyles), e(window.SiteHeader, {
    page: 'free-tools',
    lang: 'en'
  }), e('main', {
    className: 'ft-main'
  ...
  e('section', {
    ...
    className: 'ft-hero__rule',
    'aria-hidden': 'true'
  }), e('div', {
    className: 'site-container ft-hero__in'
  }, e('div', {
    className: 'ft-hero__copy'
  }, e('p', {
    className: 'ft-eyebrow'
  }, 'Free tools'), e('h1', {
    className: 'ft-hero__h1'
  }, 'Useful tools for business, career and everything going on in your head.'), e('div', {
    className: 'ft-hero__tick',
    'aria-hidden': 'true'
  }), e('p', {
    className: 'ft-hero__lead'
  }, "Diagnostics, feedback, exercises and other things I've built to help you figure something out or make something better."), e('p', {
    className: 'ft-hero__note'
  }, 'The tools that are live are free to use immediately.')), e('figure', {
    className: 'ft-hero__fig',
    'aria-hidden': 'true'
  }, e('div', {
    className: 'ft-hero__disc'
  }), e('div', {
    className: 'ft-hero__disc-cut'
  }), e('div', {
    className: 'ft-hero__h'
  }), e('div', {
  ...
  e('section', {
    ...
    className: 'ft-collection__head'
  }, e('h2', {
    className: 'ft-collection__h'
  }, 'What do you need help with?')), filter, e('div', {
    ref: liveRef,
    'aria-live': 'polite',
    role: 'status',
    style: {
  ...
  e('section', {
    ...
    className: 'ft-ask__glow',
    'aria-hidden': 'true'
  }), e('div', {
    className: 'site-container ft-ask__in'
  }, e('p', {
    className: 'ft-ask__eyebrow'
  }, 'Still not it?'), e('h2', {
    className: 'ft-ask__h'
  }, "Can't find the right tool?"), e('div', {
    className: 'ft-ask__tick',
    'aria-hidden': 'true'
  }), e('p', {
    className: 'ft-ask__p'
  }, "Some problems don't fit neatly into a questionnaire."), e('p', {
    className: 'ft-ask__p'
  }, 'Ask me anything with your name or anonymously.'), e('a', {
    className: 'pill pill--green',
    href: ASK_URL,
    onClick: function () {
      ftTrack('free_tools_ask', {});
    }
  }, e('span', null, 'ASK ME SOMETHING'), e('span', {
    'aria-hidden': 'true'
...
Object.assign(window, {
  FreeToolsPage: FreeToolsPage,
  renderFreeTools: renderFreeTools,
  FT_TOOLS: FT_TOOLS
});
