...
var FA_SCORING = {
  ...
  layers: { contextual: 0.45, universal: 0.55 },

  contextual: {
    prior: 0.25,         // PRIOR: where a thinly evidenced area is pulled towards (0–1)
    priorWeight: 1.5,    // K: how strongly, in points. Smaller = trust raw answers more
  ...
  universal: {
    ...
    items: {
      u01: { polarity: 'positive', areas: { direction: 1 } },
      u02: { polarity: 'positive', areas: { direction: 1 } },
      u03: { polarity: 'positive', areas: { self_trust: 1 } },
      u04: { polarity: 'problem', areas: { self_trust: 1 } },
      u05: { polarity: 'positive', areas: { action: 1 } },
      u06: { polarity: 'problem', areas: { action: 1 } },
      // Discomfort / avoidance: shared, weighted slightly more towards Action.
      u07: { polarity: 'positive', areas: { action: 0.6, self_trust: 0.4 } },
      u08: { polarity: 'problem', areas: { action: 0.6, self_trust: 0.4 } },
      u09: { polarity: 'positive', areas: { relationships_boundaries: 1 } },
      u10: { polarity: 'positive', areas: { relationships_boundaries: 1 } },
      u11: { polarity: 'positive', areas: { capacity: 1 } },
      u12: { polarity: 'positive', areas: { capacity: 1 } },
      // Repeated-pattern signal: scores no area directly. See `pattern`.
      u13: { polarity: 'problem', areas: {} },
      u14: { polarity: 'problem', areas: { environment: 1 } },
  ...
  pattern: {
    item: 'u13',
    noSignalAtOrBelow: 2,        // "Rarely true" or less is not a repeated pattern
    environmentReduction: 0.35,  // ENV_REDUCTION: at "True", Environment keeps 65%
    internalBoost: 8,            // INTERNAL_BOOST: at "True", up to +8 points
    internalAreas: ['direction', 'action', 'self_trust', 'relationships_boundaries'],
  },

  result: {
    closeThreshold: 5,   // top two within this many points → isCloseResult
    lowSignalThreshold: 30, // primary below this → nothing stands out strongly
  },
};

// ── Engine ───────────────────────────────────────────────────────────────────
function faClamp(n, lo, hi) { return Math.max(lo, Math.min(hi, n)); }
function faRound3(n) { return n == null ? null : Math.round(n * 1000) / 1000; }
...
function faScore(input, data, config) {
  ...
  var ids = D.FOCUS_AREAS.map(function (a) { return a.id; });
  var zero = function () { var m = {}; ids.forEach(function (id) { m[id] = 0; }); return m; };

  // 1. Contextual layer
  var questions = D.contextualQuestions(input.persona, input.primaryProblem);
  var earned = zero(), possible = zero();
  ...
  questions.forEach(function (q) {
    ...
    ids.forEach(function (id) {
      var best = 0;
      q.options.forEach(function (o) { best = Math.max(best, (o.points && o.points[id]) || 0); });
      if (best <= 0) return;
      possible[id] += best;
  ...
  // 3. Combine
  var K = S.contextual.priorWeight, PRIOR = S.contextual.prior;
  var breakdown = {};
  ids.forEach(function (id) {
    ...
    breakdown[id] = {
      contextualEarned: earned[id], contextualPossible: possible[id],
      contextual: C, contextualWeight: wC,
      universal: U, universalWeight: wU,
      base: base * 100, pattern: 0, score: 0,
    };
  });
  ...
  if (patternP > 0) {
    ...
    var topInternal = 0;
    S.pattern.internalAreas.forEach(function (id) { topInternal = Math.max(topInternal, earned[id] || 0); });
    if (topInternal > 0) {
  ...
  var evidenceRatio = function (id) { return possible[id] > 0 ? earned[id] / possible[id] : 0; };
  ...
  Object.keys(breakdown).forEach(function (id) {
    var b = breakdown[id];
    ['contextual', 'contextualWeight', 'universal', 'universalWeight', 'base', 'pattern', 'score'].forEach(function (k) { b[k] = faRound3(b[k]); });
  });
