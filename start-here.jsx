...
var OFFERS = [
  { name: 'Psychotherapy / decision coaching', desc: 'Think through the decision or pattern that keeps circling.', href: '/psychotherapy-decision-coaching/', service: 'psychotherapy-decision-coaching' },
  { name: 'Career strategy consulting', desc: 'Build a paid offer while you keep your job.', href: '/career-strategy-consulting/', service: 'career-strategy-consulting' },
  { name: 'Solo business growth consulting', desc: 'Grow the business without grinding yourself down.', href: '/solopreneur-growth-consulting/', service: 'solopreneur-growth-consulting' },
];
...
var Q1 = {
  ...
  cards: [
    { text: 'I know what I want to work on.', screen: 'offers' },
    { text: "I have a problem, but I'm not sure what kind of help I need.", screen: 'orientation' },
    { text: 'I want to work something out on my own first.', screen: 'clarity' },
    { text: 'I want to see how you work before I book anything.', screen: 'wtf' },
  ],
  ask: 'Prefer to ask something privately? Ask me anything →',
};

var INITIAL = { screen: 'q1', firstAnswer: null };
...
function StartHereStyles() {
  var css = [
    ...
    '.sh-instr{font-size:16px;line-height:1.55;color:#282726;margin:0 0 30px;max-width:60ch}',
    // card grid (Step 1)
    ...
    '.sh-list{display:flex;flex-direction:column;margin-top:4px}',
    '.sh-row{display:flex;align-items:center;justify-content:space-between;gap:24px;padding:22px 4px;border-top:1px solid rgba(24,26,28,.16);color:#181A1C;text-decoration:none;transition:color .15s,border-color .15s,padding-left .15s}',
    '.sh-row:last-child{border-bottom:1px solid rgba(24,26,28,.16)}',
    '.sh-row:hover{color:#047857;border-top-color:#047857;padding-left:12px}',
    '.sh-row:focus-visible{outline:2px solid #047857;outline-offset:3px}',
    '.sh-row__main{min-width:0;display:flex;flex-direction:column;gap:6px}',
    '.sh-row__title{font-family:var(--font-heading);font-size:clamp(20px,1.5vw,24px);font-weight:600;line-height:1.22;letter-spacing:-.01em;color:inherit}',
    '.sh-row__desc{font-size:15px;line-height:1.5;color:#5E6264}',
    '.sh-row__go{flex:none;display:inline-flex;align-items:center;gap:8px;font-size:13px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#047857;white-space:nowrap}',
    '.sh-row__arrow{flex:none;font-size:20px;line-height:1;color:#047857}',
    ...
    '.sh-btn{display:inline-flex;align-items:center;justify-content:center;width:100%;min-height:54px;padding:0 20px;background:#047857;color:#fff;font-family:inherit;font-weight:700;font-size:13px;letter-spacing:.05em;text-transform:uppercase;line-height:1.25;text-align:center;border:1.5px solid #047857;border-radius:0;text-decoration:none;cursor:pointer;transition:filter .16s}',
    ...
    '.sh-textlink{background:none;border:0;padding:2px 0;font-family:inherit;font-size:15px;line-height:1.5;color:#282726;cursor:pointer;text-align:left;text-decoration:none;transition:color .15s}',
    '.sh-textlink:hover{color:#047857}',
    '.sh-textlink:focus-visible{outline:2px solid #047857;outline-offset:3px}',
    '.sh-textlink--low{font-size:14px;color:#5E6264}',
    // quiet controls
    ...
    '@media (max-width:680px){.sh-grid{grid-template-columns:1fr;gap:18px}.sh-card{min-height:0;padding:22px 24px 24px}.sh-card__top{margin-bottom:22px}.sh-stage{min-height:0}.sh-row{padding:20px 2px}}',
    '@media (prefers-reduced-motion:reduce){.sh-card,.sh-card__arrow,.sh-btn,.sh-quiet,.sh-row{transition:none}.sh-card:hover,.sh-card.is-pressing{transform:none}}',
  ].join('');
...
function StartHerePage() {
  ...
  function markFirst(i) {
    var upd = { screen: 'q1', firstAnswer: i };
  ...
  function selectCard(i) {
    ...
    track('start_here_step1', { choice: target });
    timerRef.current = setTimeout(function () {
      lockRef.current = false;
      setPending(null);
      commit({ screen: target, firstAnswer: i }, true);
    }, SELECT_DELAY);
  }

  // In-flow navigation to another next-step screen (e.g. orientation → clarity).
  function goScreen(name) {
    track('start_here_cross', { to: name });
    commit({ screen: name, firstAnswer: stRef.current.firstAnswer }, true);
  }
  ...
  R.useEffect(function () {
    ...
    if (liveRef.current) {
      var label = st.screen === 'q1'
        ? ('Step 1. ' + Q1.heading)
        : ('Your next step: ' + (headingRef.current ? headingRef.current.textContent : ''));
  ...
  function fallback(key, text, opts) {
    ...
    if (opts.href) {
      return e('a', { key: key, className: cls, href: opts.href,
        onClick: function () { track('start_here_exit', { to: key }); } }, text);
    }
    return e('button', { key: key, type: 'button', className: cls,
      onClick: function () { goScreen(opts.screen); } }, text);
  }
  ...
  function q1Screen() {
    return e('div', { className: 'sh-stage' },
      ...
      e('div', { className: 'sh-fallbacks' }, fallback('ask', Q1.ask, { href: ASK }))
    );
  }

  // Shared header for the next-step screens.
  function nextHead(heading, body) {
    return e('div', { className: 'sh-intro' },
      e('p', { className: 'sh-step' }, 'YOUR NEXT STEP'),
      e('h1', { className: 'sh-h1', tabIndex: -1, ref: headingRef }, heading),
      body ? e('p', { className: 'sh-instr' }, body) : null
    );
  }

  // A row link (offers / tools).
  function row(item, go) {
    return e('a', { key: item.href, className: 'sh-row', href: item.href,
      onClick: function () { track('start_here_open', { href: item.href }); } },
      e('span', { className: 'sh-row__main' },
        e('span', { className: 'sh-row__title' }, item.name),
        item.desc ? e('span', { className: 'sh-row__desc' }, item.desc) : null
      ),
      go
        ? e('span', { className: 'sh-row__go' }, 'Explore ', e('span', { 'aria-hidden': 'true' }, '→'))
        : e('span', { className: 'sh-row__arrow', 'aria-hidden': 'true' }, '→')
    );
  }
  ...
  function offersScreen() {
    return e('div', { className: 'sh-stage' },
      live,
      nextHead('Choose what you want help with.', null),
      e('div', { className: 'sh-list' }, OFFERS.map(function (o) { return row(o, true); })),
      e('div', { className: 'sh-fallbacks' },
        fallback('orientation', 'Still not sure which one fits? Request an orientation call →', { href: ORIENTATION })
      ),
      controls(false)
    );
  }
  ...
  function clarityScreen() {
    return e('div', { className: 'sh-stage' },
      live,
      nextHead('Start with a Clarity Tool.', 'Answer a short set of questions and get an immediate breakdown of what seems to be going on.'),
      e('div', { className: 'sh-list' }, TOOLS.map(function (o) { return row(o, false); })),
      e('div', { className: 'sh-fallbacks' },
        fallback('orientation', 'Rather talk it through? Request an orientation call →', { href: ORIENTATION }),
        fallback('ask', 'Have a question instead? Ask me anything →', { href: ASK, low: true })
      ),
      controls(false)
    );
  }
    ...
    return e('div', { className: 'sh-stage' },
      ...
      e('div', { className: 'sh-outcome' },
        e('div', { className: 'sh-outcome__lead' },
          e('p', { className: 'sh-step' }, 'YOUR NEXT STEP'),
          e('h1', { className: 'sh-h1', tabIndex: -1, ref: headingRef }, 'We can find the starting point together.'),
          e('p', { className: 'sh-outcome__body' }, 'We can have a 30-minute conversation to understand what is going on and decide which kind of help, if any, makes sense.')
        ),
        e('div', { className: 'sh-panel' },
          e('div', { className: 'sh-panel__group' },
            e('a', { className: 'sh-btn', href: ORIENTATION, onClick: function () { track('start_here_exit', { to: 'orientation' }); } }, 'REQUEST AN ORIENTATION CALL →'),
            e('p', { className: 'sh-note' }, 'Free · 30 minutes · Online')
          ),
          e('div', { className: 'sh-panel__group' },
            e('button', { type: 'button', className: 'sh-btn sh-btn--ghost', onClick: function () { goScreen('clarity'); } }, "I'D RATHER FIGURE IT OUT MYSELF FIRST →"),
            e('p', { className: 'sh-note' },
              fallback('ask', 'Not ready for a call? Ask me something anonymously →', { href: ASK, low: true })
            )
          )
        )
      ),
  ...
  function wtfScreen() {
    return e('div', { className: 'sh-stage' },
      ...
      e('div', { className: 'sh-outcome' },
        e('div', { className: 'sh-outcome__lead' },
          e('p', { className: 'sh-step' }, 'YOUR NEXT STEP'),
          e('h1', { className: 'sh-h1', tabIndex: -1, ref: headingRef }, 'Bring something real.'),
          e('p', { className: 'sh-outcome__body' }, "WTF Friday is a free weekly office hour. Bring a business, career or personal problem and we'll work on it live.")
        ),
        e('div', { className: 'sh-panel' },
          e('div', { className: 'sh-panel__group' },
            e('a', { className: 'sh-btn', href: WTF, onClick: function () { track('start_here_exit', { to: 'wtf' }); } }, 'JOIN WTF FRIDAY →'),
            e('p', { className: 'sh-note' }, 'No pitch. No need to prepare a polished question.')
          ),
          e('div', { className: 'sh-panel__group' },
            e('button', { type: 'button', className: 'sh-btn sh-btn--ghost', onClick: function () { goScreen('clarity'); } }, 'PREFER TO START PRIVATELY? EXPLORE THE CLARITY TOOLS →'),
            e('p', { className: 'sh-note' },
              fallback('ask', 'Prefer not to bring it to a group? Ask me anything →', { href: ASK, low: true })
            )
          )
        )
      ),
  ...
  var body;
  if (st.screen === 'offers') body = offersScreen();
  else if (st.screen === 'orientation') body = orientationScreen();
  else if (st.screen === 'clarity') body = clarityScreen();
  else if (st.screen === 'wtf') body = wtfScreen();
  else body = q1Screen();
