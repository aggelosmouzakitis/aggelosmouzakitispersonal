...
| 5 | `/career-transition-therapy/` was `noindex` and serving a stale prerendered snapshot showing the *old* site navigation (it was missing from the prerender list). | 1 page |
...
- **Claim consistency:** "500+ companies" replaced with the approved "100+ technology companies" across visible copy, metadata and schema (13 source occurrences + `/book/` schema + the page-shell generator, which also still emitted "Licensed Psychotherapist").
...
2. **`/career-transition-therapy/` was made indexable — on a premise that turned out to be wrong, and is now awaiting a decision.** I reported it as "linked from four live indexed pages". Re-checked with a stricter grep: the matches were inside an inline `<script>` (a `SPECIALTY_IDS` array and a `URL_MAP` config object), not anchors. **There is not one crawlable `<a href>` to it anywhere in the built site.** The page is real — 1,204 words, distinct intent, correct headings, and it links out to `/work-with-me/` — but it is currently an orphan reachable only through the sitemap. See the report in the session notes; no further change made pending a decision.
3. **The "500+ companies" claim is now "100+ technology companies" everywhere.** The only place the repo recorded 500+ was `site-unification/01-current-state.md`, a two-versions-ago migration note that lists it as a flat "verified fact" and never mentions 100+ at all. Nothing in the repository establishes it as a separate metric from the approved figure, so it was replaced in all 13 live occurrences (9 English, 3 Greek, 1 generator) plus the `/book/` Person schema and the page-shell generator. `seo-check` now fails if it reappears. The historical migration notes under `site-unification/` keep the old string as a dated record.
